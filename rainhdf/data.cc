...
/// Construct a new data layer containing integer data
data::data(
      const base& parent
    , bool is_quality
    , size_t index
  ...
  , size_(dims[0] * dims[1])
{
  ...
  new_att(hnd_what_, atn_no_data, (double) no_data);
  new_att(hnd_what_, atn_undetect, (double) undetect);
  ...
  hid_handle hnd_data(hid_data, hnd_this_, dat_data, H5T_NATIVE_INT, space, plist, create);
  new_att(hnd_data, atn_class, val_class);
  new_att(hnd_data, atn_image_version, val_image_version);

  // Write the actual image data
  if (H5Dwrite(hnd_data, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, raw) < 0)
    throw error("Failed to write data");
}

/// Construct a new data layer containing floating point data
data::data(
      const base& parent
    , bool is_quality
    , size_t index
...
void data::read(int* raw, int& no_data, int& undetect) const
{
  ...
  if (H5Dread(hnd_data, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, raw) < 0)
...
void data::write(const int* raw, int no_data, int undetect)
{
      ...
      || std::fabs(offset_) > 0.000001)
  {
    ...
    if (H5Dwrite(hnd_data, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &vec_data[0]) < 0)
      throw error("Failed to write data");
  }
  else
  {
